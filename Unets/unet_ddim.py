...
def conv2d(
    ...
    # if k > 1:
    #     tiled_weight = conv.weight.data.repeat(k, 1, 1, 1)
    #     conv.weight = nn.Parameter(tiled_weight)

    #     if bias and conv.bias is not None:
    #         tiled_bias = conv.bias.data.repeat(k)
    #         conv.bias = nn.Parameter(tiled_bias)
...
class UNet(nn.Module):
    def __init__(
        ...
        self.out = nn.Sequential(
            ...
            Swish(),
            conv2d(in_channel, 3 * (fold ** 2)*self.k, 3, padding=1, scale=1e-10 , k=self.k),
        )
    ...
    def forward(self, input, time):
        ...
        out = self.out(out)
       # print (f'this is the weight matrix of {out.shape}' )
